...
describe("2095. Delete the Middle Node of a Linked List - Medium", () => {
    ...
    test("Input1: head = [1,3,4,7,1,2,6]", () => {
        const head = ListNode.createFromArray([1, 3, 4, 7, 1, 2, 6]);
        const newHead = solution(head);
        const result = ListNode.toArray(newHead);
        expect(result).toEqual([1, 3, 4, 1, 2, 6]);
    });

    test("Input2: head = [1,2,3,4]", () => {
    ...
    test("Input3: head = [2,1]", () => {
        const head = ListNode.createFromArray([2, 1]);
        const newHead = solution(head);
        const result = ListNode.toArray(newHead);
