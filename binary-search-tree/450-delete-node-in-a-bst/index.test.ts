...
describe("700. Search in a Binary Search Tree - Easy", () => {
    ...
    test("Input1: root = [5,3,6,2,4,null,7],  val = 3", () => {
        const root = BinaryTreeNode.createFromArray([5, 3, 6, 2, 4, null, 7]);
        const val = 3;
        const result = solution(root, val);
        expect(BinaryTreeNode.toArray(result)).toBeOneOf([[5, 4, 6, 2, null, null, 7], [5, 2, 6, null, 4, null, 7]]);
    });

    test("Input2: root =[5,3,6,2,4,null,7],  val = 0", () => {
        const root = BinaryTreeNode.createFromArray([5, 3, 6, 2, 4, null, 7]);
        const val = 0;
        const result = solution(root, val);
        expect(BinaryTreeNode.toArray(result)).toEqual([5, 3, 6, 2, 4, null, 7]);
    });

    test("Input3: root =[],  val = 0", () => {
